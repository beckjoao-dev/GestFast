...
export async function GET() {
  try {
    ...
    const ingredientCount = await prisma.ingredient.count({ where: { userId: session.userId } })
    ...
    for (const { product } of calcs) {
      if (product.marginPct < 20)
        alerts.push({ type: 'danger', message: `"${product.name}": margem crítica (${product.marginPct}%). Risco de prejuízo.` })
      else if (product.marginPct < 35)
        alerts.push({ type: 'warn', message: `"${product.name}": margem baixa (${product.marginPct}%). Considere ajustar o preço.` })
    }
